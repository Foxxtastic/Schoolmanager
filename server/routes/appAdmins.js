...
const dataaccess = require('../dataaccess/appAdmins');
    ...
    async (req, res, next) => {
        try {
            ...
            const admins = await (pageNumber && pageSize ?
                ...
                dataaccess.listAllAppAdmins(sorting, isAscending, filterProperty, filterValue));
            res.json(admins);
        } catch (error) {
            next(error);
        }
...
router.get('/:userId',
    ...
    async (req, res) => {
        ...
        const admin = await dataaccess.getAppAdminByUserId(userId);
...
router.post('/',
    ...
    async (req, res, next) => {
        try {
            ...
            const admin = await dataaccess.createAppAdmin(adminDto);
            res.json(admin);
        } catch (error) {
            next(error);
        }
...
router.put('/:id',
    ...
    async (req, res, next) => {
        ...
        try {
            const admin = await dataaccess.updateAppAdmin(idAsNumber, adminDto);
            res.json(admin);
        } catch (error) {
            next(error);
        }
